...
void RafDb::Init() {
  // 初始化WAL
  if (FLAGS_enable_wal) {
    wal_dir_ = FLAGS_wal_dir;
    wal_.reset(new WAL(wal_dir_));
    if (!wal_->Init()) {
      LOG(ERROR) << "Failed to initialize WAL, disable WAL mode";
      wal_.reset();
    } else {
      // 恢复WAL日志
      uint64_t last_index, last_term;
      if (wal_->Recovery(&last_index, &last_term)) {
        LOG(INFO) << "WAL recovery success, last index: " << last_index << ", last term: " << last_term;
        // TODO: 将恢复的日志应用到LevelDB
      }
    }
  }
